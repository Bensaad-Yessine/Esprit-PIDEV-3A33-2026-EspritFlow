...
def build_features_from_json(data):
    ...
    # =========================================
    df['last_5_tasks_abandonment_rate'] = data.get("last_5_tasks_abandonment_rate") or 0.0
    df['last_7_days_completion_rate']   = data.get("last_7_days_completion_rate") or \
                                          data.get("completion_rate") or 0.5
    ...
    pause_freq           = data.get("pause_frequency") or 0.0
    pause_q75            = data.get("pause_frequency_threshold") or 4.0
    most_productive_hour = data.get("most_productive_hour") or int(df['start_hour'].iloc[0])
    avg_duration         = data.get("average_completion_duration_minutes") or 100.0
    abandonment_rate     = data.get("abandonment_rate") or 0.25
    ...
    df['user_stability'] = 1 - abandonment_rate
...
def predict():
    try:
        ...
        response = {
            ...
            "probability_complete": float(proba[1]),
            "probability_abandon":  float(proba[0]),
